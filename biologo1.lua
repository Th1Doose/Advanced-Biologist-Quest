quest biologo1 begin
	state start begin
		...
		when login or levelup with pc.get_level() >= 30 begin
			set_state("orc_biolog_1")
		end
	end

	state orc_biolog_1 begin
		when letter begin
			send_letter("First Biologist")
		end
		...
		when button or info begin
			...
			say("I'm here to ask for 10 orc tooth.")
			say("Are you willing to do that for me?")
			local s = select("Yes","No")
			if s == 1 then
				set_state("orc_biolog_2")
				clear_letter()
				pc.setqf("tooth_count", 10)
			elseif s == 2 then
				return
			end
		end
	...
	state orc_biolog_2 begin
		...
		when button or info begin
			...
			local s = select("Yes","No")
			if s == 1 then
				if pc.getqf("tooth_deliver_delay") > get_global_time() then
					say_title("Time")
					say("You need to wait 15 minutes.")
					say("")
					say_reward(string.format(biologo1.secondsToTime(pc.getqf("tooth_deliver_delay"), "Remaining time: ", "")))
					return
				elseif pc.count_item(30006) < 1 then
					say_title("Orc Tooth")
					say("You don't have Orc Tooth.")
				elseif pc.count_item(30006) >= 1 then
					if pctg == 1 then
						pc.setqf("tooth_count", pc.getqf("tooth_count") - 1)
						if pc.getqf("tooth_count") >= 1 then
							say_title("Orc Tooth")
							say("This tooth is in perfect condition.")
							pc.remove_item(30006, 1)
							say_reward(string.format("There is "..pc.getqf("tooth_count").." orc tooth left to deliver."))
							pc.setqf("tooth_deliver_delay", get_global_time() + 60*15)
						else
							say_title("Orc Tooth")
					...
					elseif pctg != 1 then
						...
						pc.remove_item(30006, 1)
						say_reward(string.format("Left to deliver: "..pc.getqf("tooth_count").." orc tooth."))
						pc.setqf("tooth_deliver_delay", get_global_time() + 60*15)
					end
				end
			--elseif s == 2 then
			...
			--	elseif pc.getqf("tooth_deliver_delay") > 0 then
			--		pc.setqf("tooth_deliver_delay", get_global_time() * 0)
